...
func (qs *queueTestSuite) TestSendBatch() {
	msgs := make([]*gosoSqs.Message, 0)

	// Test case 1: empty batch
	err := qs.queue.SendBatch(context.Background(), msgs)
	if !qs.Nil(err) {
		qs.FailNow("empty messages slice should return nil and not call any other func")
	}

	// Test case 2: 1 message in batch, but triggers a request too long error
	...
	qs.executor.
		...
		Return(nil, awsErr)
	err = qs.queue.SendBatch(context.Background(), msgs)
	if !qs.Equal(awsErr, err) {
		qs.FailNow("SendBatch should fail with only one message which triggers a request too long error")
	}

	// Test case 3: 3 messages in batch, but triggers a request too long error
	msgs = append(msgs, &gosoSqs.Message{
		DelaySeconds:           aws.Int64(int64(1)),
	...
	err = qs.queue.SendBatch(context.Background(), msgs)
	if !qs.Nil(err) {
		qs.FailNow("SendBatch should succeed by splitting the messages in smaller chunks and calling itself again with each of those chunks")
	}
}
